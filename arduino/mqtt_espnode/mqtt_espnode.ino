...
char nodeName[16] = "espnode";                  // Nodes name - default value, may be overridden
char uniqueNodeName[50] = "";                   // Unique node name, generated by combining node name and parts of the mac address
...
bool shouldSaveConfig = false;                  // Flag indicating the configuration should be saved
...
const char defaultTopicBase[16] = "espnodes/";  // MQTT Base for default topic
...
const char HTML_SETTINGS_NODE_NAME[] PROGMEM = "<b>Node Name</b> <i><small>(required. lowercase letters, numbers, and _ only)</small></i><input id='nodeName' required name='nodeName' maxlength=15 placeholder='Node Name' pattern='[a-z0-9_]*' value='{nodeName}'>";
...
String mqttGetDefaultTopic() {
    String topic = defaultTopicBase + String(uniqueNodeName);
    return topic;
}
...
boolean mqttSend(String topic, String cmd) {
...
void webHandleSaveSettings() {
    ...
    if (webServer.arg(String(F("nodeName"))) != String(nodeName)) {
        ...
        lowerNodeName.toCharArray(nodeName, 16);
    }
    ...
    if (shouldSaveConfig || shouldSaveWifi) {
